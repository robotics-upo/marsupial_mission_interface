...
/**                                                                    */   
/***********************************************************************/
#ifndef _BISECTION_CAT_3D_H_
#define _BISECTION_CAT_3D_H_
...
class bisectionCat
{
    public:
        ...
        bool configBisection(double _l, double _x1, double _y1, double _z1, double _x2, double _y2, double _z2);
        double resolveBisection(double a1_, double b1_, int mode_);// 0 = to find phi() , 1 = to find X0 , 2 = to find Y0
        //Find points with sign changes in interval a-b, times that function pass through the origin 
        bool lookingSignChanging (double a, double b, int mode_);
        double functionBisection(double xr_aux, int mode_);
        void getPointCatenary3D(vector<geometry_msgs::Point> &_v_p, bool dist_interpolation_ = true);
        void getPointCatenaryStraight(vector<geometry_msgs::Point> &_v_p);
        void resetVariables();
        void setFactorBisection(double _fa,double _fb);
        void checkStateCatenary(double _x1, double _y1, double _z1, double _x2, double _y2, double _z2);
        void getNumberPointsCatenary(double _length);
        bool setNumPointsPerUnitLength(int n);
        void setResolution(int res_);
        void getMinPointZCat(geometry_msgs::Point &p_, int &n_);
        void getMidPointCat(geometry_msgs::Point &p_, int &n_);
        void getStatusCollisionCat(std::vector<double> &dist_obst_cat_, std::vector<int> &pos_cat_in_coll_, 
        ...
        void markerPoints(visualization_msgs::MarkerArray _marker, std::vector<geometry_msgs::Point> _vector, ros::Publisher c_m_pub_);
